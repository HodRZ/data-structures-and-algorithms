...
const listFoods = (recipe) => {
  let result = [];
  const ing = recipe.ingredients.slice(',');
  ing.forEach((item) => {
    const firstSlice = item.slice(item.indexOf(' '));
    const secondSlice = firstSlice.slice(firstSlice.indexOf(' ') + 1);
    const thirdSlice = secondSlice.slice(secondSlice.indexOf(' ') + 1);
    result.push(thirdSlice);
  })
  return result;
};
