...
const count = (target, input) => {
  const flatted = input.flat()
  return flatted.reduce((acc, num) => {
    if (num === target) acc++
    return acc
  }, 0)
}
...
const divisibleByFiveTwoToThePower = (input) => {
  const numsRaised = input.map((nums) => {
    const filtered = nums.filter((num) => (typeof (num) === 'number' && num % 5 === 0))
    return filtered.reduce((acc, num) => {
...
let findMaleAndFemale = (data) => {
  const filtered = data.reduce((list, char) => {
    if (char.gender === 'male' || char.gender === 'female') list.push(char.name)
    return list
  }, [])
